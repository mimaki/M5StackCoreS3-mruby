...
#define PRINTF(...) m5printf(__VA_ARGS__)
// #define FPUTS(s, _o) M5.Lcd.printf(s)
#define FPUTS(s, _o) m5fputs(s, _o)
// #define PUTC(c, _o) M5.Lcd.printf("%c", c)
#define PUTC(c, _o) m5putchar(c)
...
mirb(mrb_state *mrb)
{
  ...
  //   uart_read_bytes(UART_NUM_0, &buf, 16, pdMS_TO_TICKS(1000));
  //   M5.Lcd.printf("Input: %s\n", buf);
  //   M5.update();
  // }
